...
import { stripDeveloperInstructions, tldrSystemPrompt, workspacePeersMcpPrompt } from "../shared/agent-prompts";
...
const peersToolNames = [`mcp__${peersServerName}__list_sessions`, `mcp__${peersServerName}__read_session`];
...
function installPeersShim(): void {
  ...
  const script = [
    ...
    `exec env ELECTRON_RUN_AS_NODE=1 ${JSON.stringify(process.execPath)} ${JSON.stringify(peersEntryPath())} --threads ${JSON.stringify(threadsStorePath())} "$@"`,
    "",
  ].join("\n");
 ...
 * The tools are read-only, so they are allow-listed: a headless `-p` session
 * cannot answer a permission prompt, and an unapproved tool is a denied tool.
 */
  ...
  const config = {
    mcpServers: {
      [peersServerName]: {
        ...
        args: [peersEntryPath(), "--mcp", "--threads", threadsStorePath(), "--cwd", cwd, "--self", sessionId],
...
function mergeLoadedConversations(...results: ClaudeConversationResult[]): ClaudeConversationResult {
...
function taskOutputSignature(items: ConversationItem[]): string | undefined {
  const signatures = items.flatMap((item) => {
    ...
    try {
      const { mtimeMs, size } = statSync(agent.outputFile);
      return [`${agent.outputFile}:${size}:${mtimeMs}`];
    } catch {
      return [`${agent.outputFile}:missing`];
    }
  });
...
async function loadSessionFileChanges(request: SessionFileChangesRequest): Promise<SessionFileChanges> {
  const touched = collectEditedPaths(readConversation(request).items);
