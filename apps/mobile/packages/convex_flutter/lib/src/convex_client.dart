...
class ConvexClient {
  ...
  /// await client.setAuth(token: '<jwt-from-your-auth-provider>');
  ///
  /// // Clear auth
  ...
  /// - On a bounded backoff schedule after a failed fetch (`null` return);
  ///   the refresh loop retries instead of stopping, and only clears auth once
  ///   the previously applied token has actually expired
  ///
  /// [onAuthChange] fires `true` after EVERY successful token application (a
  /// per-refresh heartbeat, not a once-ever signal) and `false` when auth is
  /// actually cleared. Re-arming (calling this again) supersedes the previous
  /// refresh loop atomically — a dying loop can never clear or overwrite the
  /// successor's auth.
  ///
  /// Example usage:
  /// ```dart
  ...
  /// Disposes the singleton and clears it so [initialize] can be called again.
  ///
  /// Intended for environment-switching in debug/dev builds only.
