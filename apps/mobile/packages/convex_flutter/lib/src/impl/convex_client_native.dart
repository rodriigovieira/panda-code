...
class NativeConvexClient implements IConvexClient {
  ...
  /// flutter_rust_bridge throws if init() is called more than once per process.
  /// Guard it so env-switch restarts (which recreate NativeConvexClient) work.
  ...
  static Future<NativeConvexClient> create(ConvexConfig config) async {
    // Initialize Rust FFI library — only safe to call once per process.
  ...
  Future<String> query(String name, Map<String, String> args) async {
    // Args are already Map<String, String> with JSON-encoded values,
    // so pass directly without buildArgs() to avoid double-encoding.
    return await _rustClient
        .query(name: name, args: args)
