...
import 'package:convex_flutter/src/utils.dart' show stripNulls;
...
class WebConvexClient implements IConvexClient {
  ...
  void _sendAuthMessage(String token) {
    try {
      ...
      _sendMessage({'type': 'Authenticate', 'token': token});
  ...
  Future<String> mutation({
    ...
    try {
      ...
      // Strip null values so Convex treats them as undefined (v.optional compatibility)
      final cleanedArgs = stripNulls(args) as Map<String, dynamic>;
      _sendMessage({
        'type': 'Mutation',
        'requestId': requestId,
        'udfPath': name, // Use udfPath instead of name
        'args': [cleanedArgs], // Args must be array, not object
      });
  ...
  Future<AuthHandle> setAuthWithRefresh({
    ...
    // Web: token refresh uses polling (fetch once per call for now)
